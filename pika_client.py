...
class PikaClient:
    ...
    async def send_message(self,queue_name:str,message: dict):
        connection = await aio_pika.connect_robust(host=env.get('RABBIT_HOST', '127.0.0.1'),port=5672)
    ...
    async def consume(self,loop,queue_name:str,callback):
        connection = await aio_pika.connect_robust(host=env.get('RABBIT_HOST', '127.0.0.1'),port=5672,loop=loop)
        channel = await connection.channel()
